...
def test_notification_hook_present():
    ...
    service_path = (
        Path(__file__).parent / "appmodules" / "services" / "notification_service.py"
    )
    if not service_path.exists():
        print("  ❌ notification_service.py não encontrado")
        assert False, "notification_service.py não encontrado"
    ...
    if missing:
        print(f"  ❌ Trechos ausentes em notification_service.py: {missing}")
        assert False, f"Trechos ausentes: {missing}"

    print("  ✓ Serviço de notificação e funções esperadas presentes")
...
def test_validador_os():
    ...
    dados_desc_curta = {"nome_solicitante": "João", "descricao": "Curto"}
...
def test_validador_usuario():
    ...
    class ValidadorUsuario:
        ...
        def validar_cadastro(
            username: str, password: str, confirm: str = None
        ) -> ValidacaoResultado:
            erros = []
...
def main():
    ...
    for nome, func in [
        ...
        ("Notification Hook", test_notification_hook_present),
        ("ValidadorOS", test_validador_os),
        ("ValidadorUsuario", test_validador_usuario),
    ]:
        try:
            resultado = func()
