...
def test_extrair_comando_status():
    ...
    for mensagem, esperado in testes:
        ...
        assert resultado == esperado, (
            f"Falhou para '{mensagem}': {resultado} != {esperado}"
        )
...
def test_extrair_numero_whatsapp():
    ...
    for numero, esperado in testes:
        ...
        assert resultado == esperado, (
            f"Falhou para '{numero}': {resultado} != {esperado}"
        )
...
def test_processar_mensagem_sem_comando():
    ...
    assert resultado["tipo"] == "mensagem_livre", (
        f"Tipo incorreto: {resultado.get('tipo')}"
    )
    assert "ajuda" in resultado["resposta"].lower(), (
        f"Resposta não contém 'ajuda': {resultado['resposta']}"
    )
    print("  ✓ Mensagem sem comando processada")
...
def test_processar_comando_status():
    ...
    print("  ✓ Comando status processado")
...
def test_validar_remetente_autorizado():
    ...
    print("  ✓ Validação de remetente funcionando")
...
def test_gerar_mensagem_ajuda():
    ...
    print("  ✓ Mensagem de ajuda gerada com sucesso")
