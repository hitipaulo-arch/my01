...
def _valor_form_ou_original(
    form_value: str, original_data: dict, *chaves: str, default: str = ""
) -> str:
...
def qr_formulario_pdf():
    ...
    return send_file(
        ...
        as_attachment=False,
        download_name="qr-formulario.pdf",
    )
...
def criar_os():
    ...
    try:
        ...
        logger.info(
            f"Nova OS (Pedido #{numero_pedido}) adicionada por: {os_data.solicitante}"
        )
...
def os_abertas():
    ...
    try:
        ...
        for item in finalizadas:
            ts_base = _parse_datetime_maybe_time(
                item.get("Carimbo de data/hora", ""), None
            )
            inicio_txt = item.get("Horario de Andamento", "") or item.get(
                "Horario de Inicio", ""
            )
        ...
        return render_route_error(
            e,
            template_name="os_abertas.html",
            user_message="Erro ao carregar lista de OS abertas.",
            extra_context={
                "chamados": [],
                "total_chamados": 0,
                **empty_metrics,
...
def atualizar_chamado():
    ...
    try:
        ...
        status_os = _valor_form_ou_original(
            request.form.get("status_os", ""), os_original, "Status da OS", default=""
        )
        ...
        numero_pedido = _primeiro_valor_disponivel(
            os_original, "ID", "/", default=str(row_id)
        )
        ...
        # Calcula horas trabalhadas se andamento e término estiverem preenchidos.
        if horario_andamento and horario_termino:
            try:
                # Tenta múltiplos formatos para garantir compatibilidade
                fmt_andamento = (
                    "%d/%m/%Y %H:%M:%S"
                    if len(horario_andamento) > 10
                    else "%d/%m/%Y"
                )
                fmt_termino = (
                    "%d/%m/%Y %H:%M:%S" if len(horario_termino) > 10 else "%d/%m/%Y"
                )
                dt_andamento = datetime.datetime.strptime(horario_andamento, fmt_andamento)
                dt_termino = datetime.datetime.strptime(horario_termino, fmt_termino)

                if dt_termino > dt_andamento:
                    delta = dt_termino - dt_andamento
                ...
                logger.warning(
                    f"Não foi possível calcular horas trabalhadas para OS #{numero_pedido}: {e}"
                )
        ...
        if status_novo_norm == "finalizada" and status_original_norm != "finalizada":
            try:
                ...
                if enfileirado:
                    logger.info(
                        "Notificação de finalização enfileirada com sucesso para OS #%s",
                        numero_pedido,
                    )
                else:
                    logger.warning(
                        "Notificação de finalização não foi enfileirada para OS #%s",
                        numero_pedido,
                    )
            except Exception as e:
                logger.error(
                    "Erro ao enviar WhatsApp de finalização da OS #%s: %s",
                    os_original.get("ID", row_id),
                    e,
                )
    ...
    if pedido_buscado:
        try:
            ...
            if os_data:
                ...
                resultado = {
                    "erro": f"Pedido número '{pedido_buscado}' não encontrado."
                }
        except Exception as e:
