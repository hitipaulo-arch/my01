...
class ValidadorOS:
    ...
    def validar_formulario(form_data: Dict[str, Any]) -> ValidacaoResultado:
        ...
        if not descricao:
            ...
            erros.append(
                f"Descrição deve ter pelo menos {ValidadorOS.MIN_DESCRICAO_LENGTH} caracteres."
            )
...
class ValidadorUsuario:
    ...
    def validar_cadastro(
        username: str, password: str, confirm_password: str = None
    ) -> ValidacaoResultado:
        ...
        if len(username) < ValidadorUsuario.MIN_USERNAME_LENGTH:
            erros.append(
                f"Usuário deve ter no mínimo {ValidadorUsuario.MIN_USERNAME_LENGTH} caracteres."
            )

        if len(password) < ValidadorUsuario.MIN_PASSWORD_LENGTH:
            erros.append(
                f"Senha deve ter no mínimo {ValidadorUsuario.MIN_PASSWORD_LENGTH} caracteres."
            )
        else:
            # Validar complexidade da senha
            if ValidadorUsuario.PASSWORD_MUST_HAVE_UPPERCASE and not any(
                c.isupper() for c in password
            ):
                erros.append("Senha deve conter pelo menos 1 letra maiúscula.")
            if ValidadorUsuario.PASSWORD_MUST_HAVE_DIGIT and not any(
                c.isdigit() for c in password
            ):
                erros.append("Senha deve conter pelo menos 1 número.")
            if ValidadorUsuario.PASSWORD_MUST_HAVE_SPECIAL and not any(
