...
class NotificationService:
    ...
    def enviar_email(
        ...
        enabled = os.getenv("NOTIFY_ENABLED", "false").strip().lower() in (
            "1",
            "true",
            "yes",
            "on",
        ...
        if not to_raw or not smtp_host:
            logger.warning(
                "Email habilitado, mas SMTP_RECIPIENTS ou SMTP_HOST não configurados"
            )
            return False
        ...
        try:
            ...
            logger.info(
                f"Email enviado para: {', '.join(recipients)} (OS #{numero_pedido})"
            )
            return True
        except Exception as e:
    ...
    def notificar_finalizacao_os(
        ...
        to_number = NotificationService._normalizar_destino_whatsapp(
            whatsapp_solicitante
        )
        if not to_number:
            logger.warning(
                "Número de WhatsApp do solicitante inválido para OS #%s", numero_pedido
            )
            return False

        mensagem = (
            f"Sua OS #{numero_pedido} já está terminada, agradecemos seu contato!!"
        )
        logger.info(
            "Iniciando notificação de finalização da OS #%s para %s",
            numero_pedido,
            to_number,
        )
        ...
        try:
            ...
            result_web = service_web.enviar_mensagem_direta(
                phone_to=to_number, message=mensagem
            )
            if result_web.get("success", False):
                logger.info(
                    "WhatsApp de finalização enviado via WhatsApp Web para %s (OS #%s)",
                    to_number,
                    numero_pedido,
                )
                return True
            ...
            logger.warning(
                ...
                numero_pedido,
                to_number,
                result_web.get("error")
                or result_web.get("message")
            ...
            if opened:
                logger.info(
                    "Fallback click-to-chat aberto para %s (OS #%s, link=%s)",
                    to_number,
                    numero_pedido,
                    result_chat,
                )
            ...
            logger.error(
                "Erro ao enviar WhatsApp de finalização (OS #%s): %s", numero_pedido, e
            )
            return False
