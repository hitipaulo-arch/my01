...
class SheetsService:
    ...
    def _get_cache(self):
        """Retorna instância do Flask-Caching (Redis em prod, SimpleCache em dev)."""
        try:
            ...
            if isinstance(ext, dict):
                # Pega o primeiro valor (backend) que é o que tem .set/.get/.delete
                for backend in ext.values():
                    return backend
                return None
            return ext
        except RuntimeError:
            # Fallback para contexto sem app (ex.: testes isolados)
            return None
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                if attempt == max_retries:
                    ...
                    raise
                logger.warning(
                    f"Sheets call failed (attempt {attempt}/{max_retries}): {e}; retrying in {delay}s"
                )
                time.sleep(delay)
                delay *= backoff
        ...
        try:
            ...
            try:
                ...
                self.sheet = spreadsheet.add_worksheet(
                    title=self.sheet_tab, rows=2000, cols=20
                )
                self._retry(
            ...
            try:
                ...
                self.sheet_horario = spreadsheet.add_worksheet(
                    title=self.horario_tab, rows=1000, cols=10
                )
                self._retry(
                    self.sheet_horario.append_row,
                    [
                        "Data",
                        "Funcionário",
                        "Pedido/OS",
            ...
            try:
                ...
                try:
                    self.sheet_usuarios = spreadsheet.add_worksheet(
                        title=self.usuarios_tab, rows=1000, cols=10
                    )
                    self._retry(
                        self.sheet_usuarios.append_row,
                        ["Username", "Senha", "Role", "Data de Cadastro"],
                    )
                    ...
                    logger.warning(
                        f"Não foi possível inicializar a aba '{self.usuarios_tab}': {e}"
                    )
                ...
                try:
                    ...
                    try:
                        self.sheet_producao = spreadsheet.add_worksheet(
                            title=self.producao_tab, rows=2000, cols=20
                        )
                        self._retry(
                        ...
                        self.sheet_producao = None
                        logger.warning(
                            f"Não foi possível inicializar a aba '{self.producao_tab}': {e}"
                        )
                else:
    ...
    def _ensure_usuarios_sheet(self) -> bool:
        ...
        if not self.client:
            self.usuarios_error = (
                self.usuarios_error or "Cliente do Google Sheets indisponível"
            )
            return False
        ...
        try:
            ...
            try:
                ...
                if "Data de Cadastro" not in headers:
                    self._retry(
                        self.sheet_usuarios.update_cell,
                        1,
                        len(headers) + 1,
                        "Data de Cadastro",
                    )
    ...
    def _invalidate_os_cache(self) -> None:
        """Invalida cache compartilhado de OS após qualquer mutação."""
        cache = self._get_cache()
        if cache is not None:
            try:
                cache.delete(CACHE_KEY_OS)
                return
            except Exception as e:
                logger.warning(f"Falha ao invalidar cache OS no Redis: {e}")

    def _invalidate_producao_cache(self) -> None:
        """Invalida cache compartilhado de produção após qualquer mutação."""
        cache = self._get_cache()
        if cache is not None:
            try:
                cache.delete(CACHE_KEY_PRODUCAO)
                return
            except Exception as e:
                logger.warning(f"Falha ao invalidar cache produção no Redis: {e}")
    ...
    def _normalize_os_id(value: Any) -> str:
        ...
        Evita variações como espaços, apóstrofo inicial do Sheets, números com
        sufixo `.0` e notação científica em valores numéricos grandes.
        """
        ...
        normalizado = format(numero.normalize(), "f").rstrip("0").rstrip(".")
        return normalizado or texto
    ...
    def _build_os_list_from_values(self, data: List[List[str]]) -> List[dict]:
        ...
        for i, row in enumerate(data[1:], start=2):
            ...
            if str(os_dict.get("Status da OS", "")).strip().lower() not in (
                "cancelada",
                "cancelado",
            ):
                ...
                for id_val in ids_column or []:
                    try:
                        if id_val and str(id_val).strip():
                            ids_numericos.append(int(self._normalize_os_id(id_val)))
                    except ValueError:
                        continue
    ...
    def _ensure_whatsapp_column(self) -> None:
        ...
        try:
            ...
            logger.info(
                "Coluna '%s' adicionada na aba '%s'",
                self.WHATSAPP_HEADER,
                self.sheet_tab,
            )
        except Exception as e:
            logger.warning(
                "Falha ao garantir coluna de WhatsApp na aba '%s': %s",
                self.sheet_tab,
                e,
            )
    ...
    def _col_letter(self, idx: int) -> str:
        """Converte índice de coluna 1-based para letra(s) de coluna (ex.: 1->A, 27->AA)."""
        if idx <= 0:
            return ""
        result = ""
        while idx > 0:
    ...
    def get_all_producao(
        ...
        try:
            ...
            if (
                ...
                cached = cache.get(CACHE_KEY_PRODUCAO)
            ...
            if cache is not None:
                try:
                    cache.set(
                        CACHE_KEY_PRODUCAO,
                        producao_list,
                        timeout=self._producao_cache_ttl_seconds,
                    )
                except Exception as e:
                    logger.warning(f"Falha ao gravar cache produção: {e}")
    ...
    def get_producao_by_row_id(self, row_id: int) -> Optional[dict]:
        ...
        try:
            ...
            headers = self._normalize_producao_headers(
                self._retry(self.sheet_producao.row_values, 1)
            )
    ...
    def update_producao(self, row_id: int, row_data: list) -> bool:
        ...
        try:
            ...
            self._retry(
                self.sheet_producao.update,
                f"A{row_id}:{ultima_coluna}{row_id}",
                [row_data],
            )
            self._invalidate_producao_cache()
    ...
    def delete_producao(self, row_id: int) -> bool:
        """Deleta um item de produção pela linha da planilha."""
        try:
            if not self._ensure_producao_sheet() or row_id < 2:
                return False

            # Confirma que a linha existe e não está vazia
            row_data = self._retry(self.sheet_producao.row_values, row_id)
            if not row_data or not any(str(v).strip() for v in row_data):
                logger.warning(
                    f"Tentativa de deletar linha vazia (row_id={row_id})"
                )
                return False

            self._retry(self.sheet_producao.delete_rows, row_id)
            self._invalidate_producao_cache()
            logger.info(f"Item de produção (linha {row_id}) deletado")
            return True
        except Exception as e:
            logger.error(f"Erro ao deletar item de produção: {e}")
            return False
    ...
    def marcar_origem_padrao_producao(self, origem_padrao: str = "produção") -> int:
        ...
        try:
            ...
            self._invalidate_producao_cache()
            logger.info(
                "Migração de origem concluída: %s linhas atualizadas", total_atualizadas
            )
            ...
            if cache is not None:
                try:
                    cache.set(
                        CACHE_KEY_OS,
                        os_list,
                        timeout=self._os_cache_ttl_seconds,
                    )
                except Exception as e:
                    logger.warning(f"Falha ao gravar cache OS: {e}")
    ...
    def get_os_by_id(self, os_id: str) -> Optional[dict]:
        ...
        try:
            ...
            os_id_normalizado = self._normalize_os_id(os_id)

            for os_item in self.get_all_os(use_cache=True):
                if self._normalize_os_id(os_item.get("ID", "")) == os_id_normalizado:
                    return {
                        "id": os_item.get("ID", ""),
                        "timestamp": os_item.get("Carimbo de data/hora", ""),
                        "status": os_item.get("Status da OS", ""),
                        "descricao": os_item.get("Descrição", "")
                        or os_item.get(
                            "Descrição do Problema ou Serviço Solicitado", ""
                        ),
                    }

            cell = self._retry(self.sheet.find, os_id_normalizado, in_column=1)
            if cell:
    ...
    def add_time_record(
        ...
        try:
            ...
            self._retry(
                self.sheet_horario.append_row,
                [data, funcionario, pedido_os, tipo, horario, observacao],
            )
